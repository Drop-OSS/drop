    ...
    <li>
      ...
      >
        ...
        <div className="text-sm/6">
          ...
          <p className="text-xs font-semibold text-blue-600/80">{contributions} contribution(s)</p>
        </div>
      </Link>
    </li>
  );
...
export function Team() {
  ...
  useEffect(() => {
    (async () => {
      ...
      const team = Object.values(teamObj).sort((a, b) => b.contributions - a.contributions);

      window.localStorage.setItem("team", JSON.stringify({ team, created: Date.now() }));
      setTeam(team);
    })();
  }, []);
      ...
      <div className="mt-12 grid grid-cols-1 gap-12 lg:grid-cols-2">
        <div className="max-w-lg">
          <p className="text-sm/6 text-zinc-400">
            Drop OSS was started, mostly on a whim, in response to frustrations with the controlled
            nature of DRM games, and the missing comforts of DRM-free games. Since then, we&apos;ve
            put together a small circle of dedicated maintainers and contributors to develop Drop
            and all its amazing features.
          </p>
          <p className="mt-8 text-sm/6 text-zinc-400">
            If you know a little code, you can help out! We heavily encourage contributions,
            especially if you&apos;re passionate about the project and enjoy writing code. We use a
            variety of stacks across the various projects we maintain, but we predominantly use Vue,
            Rust, and Typescript.
          </p>
          <div className="mt-6">
            <Button
