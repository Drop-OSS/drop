...
function DropLogo() {
  return (
    <div className="relative -mb-1 inline-flex items-center justify-center gap-x-1">
      ...
      <span className="font-display text-xl font-bold text-blue-400 uppercase">Drop</span>
    </div>
  );
}
...
const projects: Array<{
  ...
  },
  {
    ...
    slug: "gamevault",
    logo: GameVaultLogo,
    description: "A source-available, mature Steam-like experience for your home server.",
  ...
  },
  {
    ...
    slug: "romm",
    logo: RomMLogo,
    description: "An open-source, self-hosted rom manager, with built-in large emulator support.",
    href: "https://romm.app/",
    highlights: [
    ...
    <div className="-m-2 grid grid-cols-1 rounded-4xl shadow-[inset_0_0_2px_1px_#ffffff4d] ring-1 ring-white/5 max-lg:mx-auto max-lg:w-full max-lg:max-w-md">
      <div className="grid grid-cols-1 rounded-4xl p-2 shadow-md shadow-black/5">
        <div className="rounded-3xl bg-zinc-900 p-10 pb-9 shadow-2xl ring-1 ring-black/5">
          <div className="flex w-full items-center justify-center pb-8">{tier.logo()}</div>
                    ...
                    {projects.map((tier) => (
                      <MenuItem key={tier.slug}>
                        <Link
                          ...
                          data-selected={tier === selectedProject ? true : undefined}
        ...
        {sections.map((section) => (
          <tbody key={section} className="group">
            ...
            {features[section].map((name) => (
              <tr key={name} className="border-b border-zinc-800 last:border-none">
                <th scope="row" className="px-0 py-4 text-sm/6 font-normal text-zinc-200">
                  {name}
                </th>
                {projects.map((project) => {
...
function FeatureItem({
  ...
  return (
    ...
    >
      ...
      {description}
      {disabled && <span className="text-right text-xs text-blue-300">coming soon&trade;</span>}
      {paid && <GameVaultPlus />}
    </li>
  );
}
